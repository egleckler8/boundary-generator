...
class Boundary:
    ...
    def __init__(self, n: int):
        ...
        corners = Boundary._generate_corners()
        corners = [(int(corner[0] * (self.n / 2)), int(corner[1] * (self.n / 2))) for corner in corners]
        boundary_set = Boundary._discrete_interpolate_corners(corners)
        self._populate_interior(boundary_set)
        ...
        print('Time elapsed in generation:', f'~{dt:.4f}s')
    ...
    def __str__(self):
        ...
        row0 = '\t'
        for x in range(self.n):
            row0 += str(self._to_grid(x)).center(3)
        s += row0 + '\n'
    ...
    def make_img(self,
        ...
        normalized_ivs = Boundary._rgb_normalize(interior_vec)
        normalized_bvs = Boundary._rgb_normalize(boundary_vec)
        ...
        for ip, idx in self.interior_idx.items():
            ...
            red_window[r][c] = int(0.5 * normalized_ivs[idx])
            green_window[r][c] = 128 - int(0.5 * normalized_ivs[idx])
            blue_window[r][c] = 128
    ...
    def _populate_interior(self, boundary_set: Set[Tuple[int, int]]) -> None:
        ...
        set" meaning: "only keep the boundary points that are needed to keep the
        interior closed."

        :param boundary_set: Set of boundary points that form a closed boundary
        :exception Exception: If ``boundary_set`` does not form a closed boundary,
        then an exception is raised and boundary generation fails.
        """
    ...
    @staticmethod
    def _generate_corners(theta_0: float = 0,
    ...
    def _discrete_interpolate_corners(corners: List[Tuple[float, float]]
        ...
        for i in range(len(corners)):
            ...
            boundary_set.update(Boundary._discrete_interpolate(a, b))
        return boundary_set

    @staticmethod
    def _rgb_normalize(values: np.ndarray) -> np.array:
        """
